...
if(submit_check('shop_submit')){
	...
	if($if_corp == 1){
		...
		//申请开通网上店铺
		if(empty($tname)) write_msg('','?m=shop&error=39');
		if(empty($cityid)) write_msg('','?m=shop&error=40');
		if(empty($address)) write_msg('','?m=shop&error=45');
		if(empty($introduce)) write_msg('','?m=shop&error=46');
		
		if(is_array($catid)){
			$db->query("DELETE FROM `{$db_mymps}member_category` WHERE userid = '$s_uid'");
			foreach($catid as $kids => $vids){
				$db->query("INSERT INTO `{$db_mymps}member_category` (userid,catid)VALUES('s_uid','$vids')");
			}
			$catids  = implode(',',$catid);
		}
		$db -> query("UPDATE `{$db_mymps}member` SET tname='$tname',levelid='1',catid='$catids',cityid='$cityid',areaid='$areaid',streetid='$streetid',introduce='$introduce',address='$address',tel='$tel',busway='$busway',mappoint='$mappoint',msn='$msn',template='$template',template='$template',web='$web',if_corp='1' $where");
