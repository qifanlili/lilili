...
if ( $action == "makeapacherewrite" )
{
    ...
    if ( !createfile( MYMPS_ROOT."/htaccess.txt", $conf ) )
    {
        write_msg( MYMPS_ROOT."/htaccess.txt 文件不可写，请检查根目录权限！" );
    }
    else
    {
        write_msg( "htaccess.txt文件更新成功！<br>Include ".str_replace( "\\", "/", MYMPS_ROOT )."/htaccess.txt", "olmsg" );
    }
    unset( $conf );
    unset( $cities );
}
