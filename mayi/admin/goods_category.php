...
if (!(submit_check(CURSCRIPT . '_submit'))) {
	...
	(empty($catname) || !(is_array($catname))) && write_msg('请填写商品分类名称');
	...
	$template = (empty($template) ? 'list' : trim($template));
	empty($catname) && write_msg('请填写商品分类名称');
	($catid == $parentid) && write_msg('隶属商品分类不能为自己！');
	$len = strlen($catname);
	(($len < 2) || (30 < $len)) && write_msg('商品分类名必须在2个至30个字符之间');
