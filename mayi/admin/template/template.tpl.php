...
<table border="0" cellspacing="0" cellpadding="0" class="vbm">
	...
	<tr bgcolor="white">
		...
		<?php foreach($template['fengge']['flag'] as $k => $v){?>
			...
			<label for="<?=$k?>">
			<div class="txt"><span class="<?=$k?>"></span><div class="clearfix"></div><input onclick="$Obj('index_blue').className='showtpl1';$Obj('index_green').className='showtpl1';$Obj('index_orange').className='showtpl1';$Obj('index_red').className='showtpl1';$Obj('index_<?=$k?>').className='showtpl1 checked';" id="<?=$k?>" name="cfg_tpl_dir" type="radio" class="radio" value="<?=$k?>" <?php if($mymps_global['cfg_tpl_dir'] == $k){?>checked="checked"<?php }?>><?=$v?> <br /><font color="#666">(<?=$k?>)</font></div>
			</label>
			</div>
		<?php }?>
		</td>
    ...
    <tr bgcolor="white">
		...
		<td class="altbg2">
		 	<input id="style_normal" name="head_style" type="radio" value="normal" <?php if($mymps_global['head_style'] == 'normal'){echo ' checked="checked" ';}?>> 传统样式
			<input id="style_standard" name="head_style" type="radio" value="new" <?php if($mymps_global['head_style'] == 'new'){?>checked="checked"<?php }?>> 左侧下拉样式
		</td>
	</tr>
    <tr bgcolor="white">
