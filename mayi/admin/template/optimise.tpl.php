  ...
  <td>
	...
	<div class="stepstart">
		<?php $lastarr = explode(',',$last);foreach($step as $k => $v){?>
			<label for="step<?php echo $k;?>"><div class="tep <?php echo in_array($k,$lastarr) ? 'on' : '';?>"><div class="chkbox"><input id="step<?php echo $k;?>" class="checkbox" name="steporder[<?php echo $k;?>]" value="1" <?php echo (in_array($k,$lastarr) || (empty($last) && $k != '2' && $k != '1')) ? 'checked="checked"' : '';?> type="checkbox"></div><?php echo $v;?></div></label>
		<?php }?>
	</div>
	<div class="clear"></div>
	<?php if($finished == 1){?>
