...
function restore_footerurl( )
{
    ...
    while ( $row = $db->fetchRow( $query ) )
    {
        ...
        $about[$row['id']]['uri'] = rewrite( "about", array(
            "part" => "aboutus",
...
function restore_headerurl( )
{
    ...
    while ( $row = $db->fetchRow( $query ) )
    {
        ...
        $category[$row['catid']]['uri'] = rewrite( "category", array(
            "catid" => $row['catid'],
            "dir_typename" => $row['dir_typename']
        ) );
    ...
    if ( is_array( $data ) )
    {
        ...
        foreach ( $data as $k => $v )
        {
            if ( $k == "news" || $k == "goods" )
            {
                $plugin[$k]['catid'] = $k;
                $plugin[$k]['flag'] = $k;
                $plugin[$k]['uri'] = rewrite( $k, array( "action" => "index" ) );
                $plugin[$k]['name'] = $v;
            }
            else
            {
