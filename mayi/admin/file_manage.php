...
if ($downfile) {
	!(is_file($downfile)) && write_msg('您要下载的文件不存在！');
	(FileExt($downfile) == 'php') && write_msg('该文件不允许下载!');
...
if ($delfile != '') {
	($part == 'template') && write_msg('模板文件不能删除，请手动在FTP目录中将其删除！');
	((FileExt($delfile) == 'php') || (FileExt($delfile) == 'css') || (FileExt($delfile) == 'js') || (FileExt($delfile) == 'html')) && write_msg('该文件不允许删除，请在FTP目录中手动删除！');
