...
if($id){
	if(!$id) errormsg('新闻ID不能为空！');
	if(!$news = $db ->getRow("SELECT * FROM {$db_mymps}news WHERE id = '$id'")) errormsg('当前新闻不存在或者已经被删除！');
	...
	if(!$cat) errormsg('您所指定的新闻栏目不存在或者已经删除！');
	...
	$page1 = page1("SELECT * FROM {$db_mymps}news WHERE catid IN($cat_children) {$city_limit} ORDER BY id DESC",$perpage);
	...
	$page1 = page1("SELECT * FROM {$db_mymps}news WHERE 1 {$city_limit} ORDER BY id DESC",$perpage);
	foreach($page1 as $kr => $r){
		$arr['begintime']   = $r['begintime'];
		$arr['hit']  		= $r['hit'];
		$arr['title']  	    = $r['title'];
