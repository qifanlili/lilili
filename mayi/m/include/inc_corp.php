...
if($catid){
	...
	$where 		= " WHERE a.status = '1' AND a.if_corp = '1'  AND a.catid IN(".get_corp_children($catid).") ";
	...
	$sql = "SELECT a.* FROM `{$db_mymps}member` AS a LEFT JOIN `{$db_mymps}member_category` AS b ON a.userid = b.userid {$where}{$city_limit_a} ORDER BY a.id DESC";
	$count_sql	= "SELECT COUNT(a.id) FROM `{$db_mymps}member` AS a LEFT JOIN {$db_mymps}member_category AS b ON a.userid = b.userid {$where}{$city_limit_a}";
