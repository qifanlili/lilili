...
if($action != 'dopost'){
	...
	$store[logo] = strstr($store[logo],'http') ? $store[logo] : $mymps_global[SiteUrl].$store[logo];
    $store[logo] = $store[logo] == $mymps_global[SiteUrl] ? '' : $store[logo];
	$store[prelogo] = strstr($store[prelogo],'http') ? $store[prelogo] : $mymps_global[SiteUrl].$store[prelogo];
    $store[prelogo] = $store[prelogo] == $mymps_global[SiteUrl] ? '' : $store[prelogo];
	$store['prelogo'] = $store['prelogo'] ? $store['prelogo'] : $mymps_global[SiteUrl].'/images/nophoto.jpg';
	$store['logo'] 	  = $store['logo'] 	  ? $store['logo'] 	  : $mymps_global[SiteUrl].'/images/nophoto.jpg';
	...
	if($part == 'about'){
		...
		foreach($page1 as $k => $row){
			...
			$arr['uri']         = Rewrite('goods',array('id'=>$row['goodsid'],'cityid'=>$row['cityid']));
