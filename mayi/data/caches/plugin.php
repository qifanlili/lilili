<?php
$data = array (
  'coupon' => 
  array (
    'id' => '1',
    'flag' => 'coupon',
    'iscore' => '0',
    'name' => '优惠券',
    'directory' => 'coupon',
    'disable' => '0',
    'ifrewrite' => NULL,
    'seotitle' => '{city}优惠券标题',
    'seokeywords' => '{city}优惠券关键词',
    'seodescription' => '{city}优惠券描述',
    'adminmenu' => '优惠券分类=coupon_category.php
已上传优惠券=coupon_list.php',
    'membermenu' => NULL,
    'version' => '1.0',
    'releasetime' => '1278232105',
    'author' => 'steven',
    'introduce' => '商铺优惠券插件',
    'siteurl' => 'http://www.mymps.com.cn',
    'email' => 'business@live.it',
    'copyright' => 'Mymps Dev.',
  ),
  'group' => 
  array (
    'id' => '2',
    'flag' => 'group',
    'iscore' => '0',
    'name' => '团购',
    'directory' => 'group',
    'disable' => '0',
    'ifrewrite' => NULL,
    'seotitle' => '{city}团购活动标题',
    'seokeywords' => '{city}团购活动关键词',
    'seodescription' => '{city}团购活动描述',
    'adminmenu' => '团购分类=group_category.php
已发起团购=group_list.php
报名管理=group_signin.php',
    'membermenu' => NULL,
    'version' => '1.0',
    'releasetime' => '1278232105',
    'author' => 'steven',
    'introduce' => '团购活动插件',
    'siteurl' => 'http://www.mymps.com.cn',
    'email' => 'business@live.it',
    'copyright' => 'MyDev.',
  ),
  'news' => 
  array (
    'id' => '3',
    'flag' => 'news',
    'iscore' => '0',
    'name' => '新闻资讯',
    'directory' => '-',
    'disable' => '0',
    'ifrewrite' => NULL,
    'seotitle' => '{city}新闻模块标题',
    'seokeywords' => '{city}新闻模块关键词',
    'seodescription' => '{city}新闻模块描述',
    'adminmenu' => '新闻管理=news.php
新闻类别=channel.php
新闻评论=news_comment.php',
    'membermenu' => NULL,
    'version' => '2.0',
    'releasetime' => '1278232105',
    'author' => 'steven',
    'introduce' => '网站新闻插件',
    'siteurl' => 'http://www.mymps.com.cn',
    'email' => 'business@live.it',
    'copyright' => 'MyDev.',
  ),
  'goods' => 
  array (
    'id' => '4',
    'flag' => 'goods',
    'iscore' => '0',
    'name' => '商品',
    'directory' => 'goods',
    'disable' => '0',
    'ifrewrite' => NULL,
    'seotitle' => '{city}商品标题',
    'seokeywords' => '{city}商品关键词',
    'seodescription' => '{city}商品描述',
    'adminmenu' => '商品分类=goods_category.php
商品管理=goods_list.php
订单管理=goods_order.php',
    'membermenu' => NULL,
    'quhuo' => '1.普通快递送货上门 
  覆盖全国800多个城市，运费5元/包裹
2.加急快递送货上门 
  支持北京、天津、上海、广州、深圳、廊坊，限当地发货订单，运费10元/包裹 
3.圆通快递 
  北京地区：运费10元/单 
4.普通邮递 
  大陆地区：运费5元/包裹，购物满29元免运费 
  港澳地区：运费为商品原价总金额的30%，最低20元 
  海外地区：运费为商品原价总金额的50%，最低50元 
5.邮政特快专递(EMS) 
  北京地区：运费为订单总金额的20%，最低16元 
  大陆其它地区：运费为订单总金额的40%，最低20元 
  港澳台地区：运费为订单总金额的50%，最低45元 
6.自提 
  支持用户上门自提，免收运费。',
    'fukuan' => '当面付款
店内交易、送货上门、预约交易均可当面付款

银行转账
可通银行转账方式将款汇款到指定账号中

网上汇款
可通网上转账方式将款汇款到指定账号中',
    'service' => '售后服务参考条文：
1、如果您购买的是数码类、手机及配件、笔记本、台式机、家电类商品，为了保证您能充分享有生产厂家提供的售后保修服务，不管您是否需要开具发票，我们都将随单为您开具，发票内容默认为您订购的商品全称，同时不支持修改发票内容。如果因为所开具的发票内容和所购商品名称不符，导致无法保修，本站概不负责。
 
2、退货时提供发票原件，超1000元现金支付的订单办理退货将不退现金。
 
3、数码类、手机及配件、笔记本、台式机、家电、打印机、扫描仪、一体机、车载GPS类商品，如果商品出现质量问题，请您自行到生产厂家售后服务中心进行检测，并开据检测报告（对于有些生产厂家售后服务中心无法提供检测报告的，需提供维修检验单据），如果检测报告确认属于质量问题，然后将检测报告、问题商品及完整包装附件，一并返回我司办理退换货手续。如有破损或丢失，我们将无法为您办理。
 
4、对于钻石、黄金、手表、珠宝首饰及个人配饰类产品，如果附带国家级宝玉石鉴定中心出具的鉴定证书的、非质量问题不予以退换货。客户在收到商品之日起（以发票日期为准）3个月内，如果出现质量问题，请自行到当地的质量监督部门-珠宝玉石质量检验中心进行检测，如果检测报告确认属于质量问题，请与本站售后服务部联系办理退换货事宜。退换货时，请您务必将检测报告、商品的外包装、内带附件、鉴定证书、说明书等随同商品一起退回。如有包装破损或缺失，扣除该商品5%的折价费；如有附件破损或缺失扣除该商品10-15%的折价费。',
    'version' => '1.0',
    'releasetime' => '1309753960',
    'author' => 'steven',
    'introduce' => '商品插件',
    'siteurl' => 'http://www.mymps.com.cn',
    'email' => 'business@live.it',
    'copyright' => 'MyDev.',
  ),
);
?>