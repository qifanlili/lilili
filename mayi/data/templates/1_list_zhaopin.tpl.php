...
<span class="ltitle">
<a href="<?=$mymps['uri']?>" target="_blank" style="<? if($mymps['ifred'] == 1) { ?>color:red;<?php } ?> <? if($mymps['ifbold'] == 1) { ?>font-weight:bold;<?php } ?>" title="<?=$mymps['title']?>"><? echo cutstr($mymps['title'],18); ?></a>
        <? if($mymps['img_count']) { ?><span class="img_count"><?=$mymps['img_count']?>图</span><?php } if($mymps['info_level'] == 2) { ?><span class="tuijian">荐</span><?php } if($mymps['certify'] == 1) { ?><span class="certify">认证</span><?php } ?>
        <div class="inforbox">
          <div class="arrow"> <em></em> <span></span> </div>
          <div class="tipsbox">
            <h3><? echo cutstr($mymps['company'],28); ?></h3>
            <ul class="lists">
              <li> <span>薪资待遇：</span> <font color="#F70"><? if($mymps['day_salary']) { echo str_replace('.00','',$mymps['day_salary']); ?><?php } else { echo str_replace('.00','',$mymps['salary']); ?>元<?php } ?></font> </li>
              <li> <span>工作地点：</span> <?=$mymps['areaname']?> </li>
              <? if($mymps['work_life']) { ?><li> <span>工作经验：</span> <? echo str_replace('.00','',$mymps['work_life']); ?> </li><?php } ?>
              <? if($mymps['education']) { ?><li> <span>学历要求：</span> <?=$mymps['education']?> </li><?php } ?>
              <? if($mymps['job']) { ?><li> <span>招聘职位：</span> <?=$mymps['job']?> </li><?php } ?>
              <? if($mymps['property']) { ?><li> <span>公司性质：</span> <?=$mymps['property']?></li><?php } ?>
              <? if($mymps['fuli']) { ?><li style="width:400px;"><?=$mymps['fuli']?></li><?php } ?>
            </ul>
            <dl class="content">
              <? echo cutstr($mymps['content'],200,'...'); ?>            </dl>
            <dl class="warning">
              <strong><?=$mymps_global['SiteName']?>提醒：</strong> 在签订合同之前，以各种理由收取非正规费用（押金、服装费、报名费、体检费、要求购买充值卡等）均有骗子嫌疑，请提高警惕。
            </dl>
            </div>
          </div>
        </span>
...
<span class="lsalary"><? if($mymps['day_salary']) { echo str_replace('.00','',$mymps['day_salary']); ?><?php } else { echo str_replace('.00','',$mymps['salary']); ?>元<?php } ?></span>
