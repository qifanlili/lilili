...
<!--栏目列表页头部广告结束--><?php if(is_array($info_list)){foreach($info_list as $mymps) { ?><div class='hover media cfix <? if($mymps['upgrade_type']>1) { ?>ding<?php } ?>'>
...
<span class='pull-rights'><span class="highlights"><? if(in_array($mymps['price'],array('0元','0.00元'))) { ?>面议<?php } else { echo (float)$mymps['price']; ?>元<?php } ?></span></span>
<a href="<?=$mymps['uri']?>" target="_blank"  style="<? if($mymps['ifred'] == 1) { ?>color:red;<?php } ?> <? if($mymps['ifbold'] == 1) { ?>font-weight:bold;<?php } ?>">[<?=$mymps['new_old']?>]<?=$mymps['title']?></a><? if($mymps['img_count']) { ?><span class="img_count"><?=$mymps['img_count']?>图</span><?php } if($mymps['info_level'] == 2) { ?><span class="tuijian">推荐</span><?php } if($mymps['certify'] == 1) { ?><span class="certify">认证</span><?php } ?><br /><font color="#808080"><? echo cutstr($mymps['content'],100); ?></font>
</div>
<div class='typo-smalls'><font class="xx1"><?=$mymps['areaname']?></font> <font class="xx2"><? echo get_format_time($mymps['begintime']); ?>发布</font> <? if($mymps['from']) { ?><font class="xx3"><?=$mymps['from']?></font><?php } ?></div>
</div>
</div>
<?php }} ?>
<!--栏目列表页尾部广告开始-->
