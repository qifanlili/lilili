...
<script type="text/javascript">uaredirect("<?=$mymps_global['SiteUrl']?>/m/index.php?cityid=<?=$cityid?>");</script>
...
<div class="focushead_right">
                        ...
                        <div class="announcecorp">
                            ...
                            <div id="tablist1">
                            	<div class="tablist block course">
                                    <ul>
                                    <?php $index_topinfo = mymps_get_infos(7,NULL,3,NULL,NULL,NULL,NULL,NULL,$cityid); ?>                                    <?php if(is_array($index_topinfo)){foreach($index_topinfo as $mymps) { ?>                                    <li><span class="title"><a target="_blank" href="<?=$mymps['uri']?>" title="<?=$mymps['title']?>" style="<? if($mymps['ifred'] ==1) { ?>color:red;<?php } if($mymps['ifbold'] ==1) { ?>font-weight:bold;<?php } ?>"><? echo cutstr($mymps['title'],42); ?></a></span><span class="money" title="<?=$mymps['catname']?>"><?=$mymps['catname']?></span></li>
                                    <?php }} else {{ ?>
        ...
        <?php $members = mymps_get_members(14,NULL,NULL,NULL,2,NULL,NULL,$cityid); ?>        <? if($members) { ?>
