...
<span class="lfaceimg"><a href='<?=$mymps['uri']?>' target='_blank'><img src='<? if(!$mymps['img_path']) { ?><?=$mymps_global['SiteUrl']?>/images/nophoto.gif<?php } else { ?><?=$mymps_global['SiteUrl']?><?=$mymps['img_path']?><?php } ?>' title='<?=$mymps['title']?>'></a></span>
<span class="ltitlevalue">
<a href="<?=$mymps['uri']?>" target="_blank" style="<? if($mymps['ifred'] == 1) { ?>color:red;<?php } ?> <? if($mymps['ifbold'] == 1) { ?>font-weight:bold;<?php } ?>" title="<?=$mymps['title']?>"><? echo cutstr($mymps['title'],42); ?></a><span class='highlight'>[<?=$mymps['position']?>]</span><? if($mymps['img_count']) { ?><span class="img_count"><?=$mymps['img_count']?>图</span><?php } if($mymps['info_level'] == 2) { ?><span class="tuijian">推荐</span><?php } if($mymps['certify'] == 1) { ?><span class="certify">认证</span><?php } ?>&nbsp;<br /><font style="font-size:12px; color:#808080"><? echo cutstr($mymps['content'],70); ?></font><br />
<font class="xx1"><?=$mymps['areaname']?></font><font class="xx2"><?=$mymps['rent_type']?></font>
</span>
<span class="ltime"><? echo get_format_time($mymps['begintime']); ?></span>
<span class="lprice"><span class="rprice"><? if(in_array($mymps['mini_rent'],array('0元','0.00元'))) { ?>面议<?php } else { echo (float)$mymps['mini_rent']; ?>元<?php } ?></span><br /><?=$mymps['room_type']?></span>
</div>
<?php }} ?>
</ul>
