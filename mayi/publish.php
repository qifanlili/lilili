...
if($act == 'dopost') {
	...
	empty($cityid) && write_msg('请选择您要发布的地区！');
	...
	if($action == 'input'){
		...
		if($yet = $db->getOne("SELECT begintime FROM `{$db_mymps}information` WHERE title='$title' AND tel = '$tel' ORDER BY id DESC")){
			if($timestamp-$yet<3600*24) write_msg('请勿重复发布信息！','?action=input&catid='.$catid);
			unset($yet);
		}
		...
		if (empty($ismember)){
			...
			if(!empty($perday_maxpost)){
				...
				$count >= $perday_maxpost && write_msg("很抱歉！您当前的会员级别每天只能发布 <b style='color:red'>".$perday_maxpost."</b> 条信息<br />如果您要继续操作，请联系客服。");
			}
		...
		if($img_count > 0){
			...
			if($img_path) $db -> query("UPDATE `{$db_mymps}information` SET img_path = '$img_path' WHERE id = '$id'");
		}
		...
		if(is_array($_FILES)){
			for($i=0;$i<count($_FILES);$i++){
				...
				if($_FILES[$name_file]['name']){
					...
					if($i===0 && $mymps_image[1]) $db -> query("UPDATE `{$db_mymps}information` SET img_path = '$mymps_image[1]' WHERE id = '$id'");
				}
			}
		}
	...
	if(!$catid){
		...
		$post['select_where_option'] = select_where_option($mymps_global[SiteUrl].'/include/selectwhere.php',$cityid,$areaid,$streetid);
		...
		$post['action']				 = "edit";
		$post['submit'] 			 = "保存修改";
		$post['select_where_option'] = select_where_option($mymps_global[SiteUrl].'/include/selectwhere.php',$post['cityid'],$post['areaid'],$post['streetid']);
		$post['mixcode']			 = md5($cookiepre);
		$cat 					 	 = post_cat_info($catid);
