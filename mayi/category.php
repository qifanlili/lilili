...
if($page > 1) $cache['list']['time'] = false;
$sq = $s = '';
...
if($cat['parentid'] == 0){
	...
	$cati=$db->getAll("SELECT catid FROM `{$db_mymps}category` WHERE parentid = '$catid'");
	foreach($cati as $k=>$v){
		$catin .= $v['catid'].',';
	}
...
foreach($page1 as $key => $val){
	...
	$ids = true;
	if($val['upgrade_time'] > 0 && $val['upgrade_time'] < $timestamp) $db->query("UPDATE `{$db_mymps}information` SET upgrade_type = '1',upgrade_time = '0' WHERE id ='$val[id]'");
...
if($adveritems) $advertisement['type']	= $advertisement['all'] ? (is_array($advertisement[$catid]['type']) ? array_merge($advertisement['all']['type'],$advertisement[$catid]['type']) : $advertisement['all']['type']): $advertisement[$catid]['type'];
switch($rewrite){
	case 'rewrite':
		...
		if($city['cityid']){
			...
			if(is_array($area_list)){
				$streetid = '';
				$area_list = array_merge(array('0'=>array('areaid'=>'0','areaname'=>'不限')),$area_list);
...
}elseif($cat['template'] == 'list_simple'){
}else{
	if($cat['modid'] > 1 && $idin) {
		$des = get_info_option_array();
		$extra = $db ->getAll("SELECT a.* FROM `{$db_mymps}information_{$cat[modid]}` AS a WHERE 1 {$idin}"); 
