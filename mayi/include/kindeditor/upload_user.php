...
if(!$member_log->chk_in()) alert("请登录用户管理中心再上传图片!");
...
if (empty($_FILES) === false) {
	...
	if($_FILES[$name_file]['name']){
		...
		$db -> query ("INSERT INTO `{$db_mymps}upload` (title,url,width,height,filesize,uptime,adminid) VALUES ('{$mymps_image[0]}','$imgsrcValue','$imgwidthValue','$imgheightValue','{$imgsize}','{$nowtime}','{$s_uid}')");
