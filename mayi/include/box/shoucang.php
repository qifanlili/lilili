...
switch($log){
	...
	if($db->getOne("SELECT COUNT(id) FROM `{$db_mymps}shoucang` WHERE infoid = '$infoid' AND userid = '$s_uid'") > 0){
		...
		$r		= $db -> getRow("SELECT title,catid,dir_typename,cityid FROM `{$db_mymps}information` WHERE id = '$infoid'");
		$url	= Rewrite('info',array('id'=>$infoid,'catid'=>$r['catid'],'cityid'=>$r['cityid'],'dir_typename'=>$r['dir_typename']));
