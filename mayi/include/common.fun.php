...
{
	global $rows_num,$page,$pages_num,$per_page,$param,$per_screen,$dir_typename;
	$font_size="10pt";
...
function globalassign(){
	...
	if(CURSCRIPT == 'store'){
		...
		$about['goods_uri'] = Rewrite('goods',array('part'=>'index'));
...
function Rewrite($types, $params){
    ...
    switch($types){
		//栏目
		case 'category':
			if($cityid && !$city) $city = get_city_caches($cityid);
        ...
        case 'info':
			...
			if($cityid && !$city[cityid]) $city = get_city_caches($cityid);
		...
		//站务
        case 'about':
			if(!$cityid || in_array($part,array('faq','aboutus'))){
				$uri = $mymps_global['SiteUrl'].'/';
			}else{
				$city = $city ? $city : get_city_caches($cityid);
				$uri = $city['domain'];
			}
		...
		case 'goods':
			...
			}elseif($id && empty($catid) && empty($orderby)){
...
function mymps_get_announce($num='12',$cityid=NULL){
	...
	while($row = $db -> fetchRow($query)){
		...
		$list[$row['id']]['begintime']	= $row['pubdate'];
		...
		$list[$row['id']]['uri'] 		= $row['redirecturl'] ? $row['redirecturl'] : Rewrite('about',array('part'=>'announce','id'=>$row['id'],'cityid'=>$cityid));
	}
	return $list;
}
...
function get_location($type = 'category',$cat = 0, $str = '', $extra='',$pdetail='')
{
	...
	if(in_array($type,array('category','channel','corp','news'))){
		...
		if($type == 'channel'){
			$location .= ' <code>'.$raquo.'</code> <a href="'.Rewrite('news',array('action'=>'index','cityid'=>$city[cityid])).'">'.$city[cityname].'热点资讯</a>';
			$page_title = $city['cityname'].'热点资讯-'.$page_title;
		} elseif($type == 'corp') {
			$location .= ' <code>'.$raquo.'</code> <a href="'.Rewrite('corp',array('action'=>'index','cityid'=>$city[cityid])).'">'.$city[cityname].'商家黄页</a>';
			$page_title = $city[cityname].'商家黄页-'.$page_title;
		}
		...
		if (!empty($cat_arr)){
			...
			foreach ($cat_arr as $val){
				...
				$location   .= ' <code> '.$raquo.' </code> <a href="' .$city['domain']. $val_uri . '">' .$city['cityname'].mhtmlspecialchars($type == 'corp' ? $val['corpname'] : $val['catname']) . '</a>';
			}
		}
...
function mymps_get_focus($type='index',$num=5,$cityid=NULL){
	...
	$city_limit = !$cityid ? " AND cityid = ''" : " AND cityid = '$cityid'";
...
function mymps_get_news($num=10,$catid=NULL,$ifimg=NULL,$leftjoin=NULL,$ifhot=NULL,$orderby=1,$cityid=NULL){
	...
	if($leftjoin){
		...
		while($row = $db -> fetchRow($query)){
			...
			$arr['caturi']		= Rewrite('news',array('catid'=>$row['catid'],'cityid'=>$row['cityid']));
...
function mymps_get_infos($num=10,$info_level=NULL,$upgrade_type=NULL,$userid=NULL,$catid=NULL,$certify=NULL,$if_hot=NULL,$tel=NULL,$cityid=NULL){
	...
	if($idin){
		$sql = "SELECT a.id,a.contact_who,a.title,a.content,a.begintime,a.catid,a.info_level,a.hit,a.dir_typename,a.ifred,a.ifbold,a.userid,a.catid,a.cityid,a.catname,a.img_path FROM `{$GLOBALS['db_mymps']}information` AS a WHERE a.id IN ($idin) {$orderby}";
		$do_mymps = $db -> query($sql);
		while($row = $db -> fetchRow($do_mymps)){
			$arr['id']        = $row['id'];
			$arr['catid']     = $row['catid'];
...
/*团购活动*/
function mymps_get_groups($num=10,$glevel=NULL,$cityid=NULL)
{
	global $db,$db_mymps;
	static $res = NULL;
	$limit = $num ? " LIMIT 0,$num" : "";
...
/*商品*/
function mymps_get_goods($num=10,$onsale=1,$shuxing=NULL,$catid=NULL,$userid=NULL,$orderby=1,$cityid=NULL)
{
	global $db,$db_mymps;
	$limit = $num ? " LIMIT 0,$num" : "";
	...
	while($row = $db -> fetchRow($query)){
		...
		$res[$row['goodsid']]['uri'] = Rewrite('goods',array('id'=>$row['goodsid'],'cityid'=>$row['cityid']));
	}
	return $res;
}
...
function select_where($table,$name='parentid',$currentid='',$parentid=''){
	...
	if($parentid){
		...
		return $view ? "<select class='input' id='$table' name=$name><option value=''>请选择</option>$view</select>" : "";
...
function select_where_option($actionfile,$cityid,$areaid,$streetid){
	...
	if(!$cityid || defined('IN_ADMIN')){
		$mymps .= '<select id=\'cityid\' name=\'cityid\'  onChange="choose_where(\'getarea\',this.options[this.selectedIndex].value,\'\',\'1\')"  class="input" style="width:140px!important" require="true" datatype="limit" msgid="地区" msg="请选择所在地区"><option value=\'\'  selected >请选择所在分站</option>';
		$mymps .= get_cityoptions($cityid);
		$mymps .= '</select>';
		$mymps .= '<span id="showarea"></span>&nbsp;<span id="showstreet"></span>';
	}else{
		$mymps .= '<input name="cityid" value="'.$cityid.'" type="hidden">';
		$mymps .= '<select id=\'areaid\' name=\'areaid\'  onChange="choose_where(\'getstreet\',this.options[this.selectedIndex].value,\'\',\'1\')"  class="input" style="width:140px!important" require="true" datatype="limit" msgid="地区" msg="请选择所在地区"><option value=\'\'  selected >请选择所在地区</option>';
		$mymps .= get_areaoptions($cityid,$areaid);
		$mymps .= '</select>';
		$mymps .= '<span id="showstreet"></span>';
	}
...
function get_areaname($areaid){
	global $db,$db_mymps;
	return $db -> getOne("SELECT areaname FROM `{$db_mymps}area` WHERE areaid = '$areaid'");
}
?>