...
function get_hot_cities(){
	global $db,$db_mymps,$mymps_global,$hotcities;
	if(!$hotcities){
...
function get_nearby_cities($provinceid=NULL){
	global $db,$db_mymps,$mymps_global;
	if($provinceid){
		$query = $db -> query("SELECT * FROM `{$db_mymps}city` WHERE provinceid = '$provinceid' AND `status` = '1' ORDER BY displayorder ASC LIMIT 0,19");
		while($row = $db -> fetchRow($query)){
			$FinalArray[$row['cityid']]['cityid'] = $row['cityid'];
			...
			$FinalArray[$row['cityid']]['domain'] = $row['domain'] ? $row['domain'] : $mymps_global['SiteUrl'].$mymps_global['cfg_citiesdir'].'/'.$row['directory'].'/';
		}
		return $FinalArray;
	}
}
...
function get_allcities($cityid=0){
	...
	if($data === false){
		...
		while($row = $db -> fetchRow($query)){
			...
			$res[$row['cityid']]['provinceid'] = $row['provinceid'];
...
function get_areaoptions($cityid=0,$areaid=0){
	global $city;
	if(!$city || !$city['cityid']) $city=get_city_caches($cityid);
	if(is_array($city['area'])){
		foreach($city['area'] as $k => $v){
			$mymps .= $areaid == $v['areaid'] ? '<option value="'.$v[areaid].'" selected>' : '<option value="'.$v[areaid].'">';
			$mymps .= $v['areaname'];
			$mymps .= '</option>';
		}
	}
